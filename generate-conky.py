...
def main():
    ...
    if load_lua_bg:
        ...
        draw_borders = False
        # Geen border color vragen bij Lua achtergrond
        color_default = "0055ff"
    else:
        # Alleen vragen als geen Lua achtergrond
        ...
        color_default = color_ask("Border (orange)", "ff981d")
    ...
    # Vraag voor plaatsing (altijd vragen, ook bij Lua)
    ...
    lines = [
    "--[[",
    "    Conky generated by Conky Generator",
    ...
    if options["cpu"]:
        ...
        lines.append("${alignc}${voffset 0}${color2}${cpubar cpu0 10,220}")
        lines.append("${alignc}${voffset 5}${cpugraph 30,220 5294E2 FF2021}")
    ...
    if options["memory"]:
        ...
        lines.append("${alignc}${voffset 0}${color2}${membar 10,220}${color}")

    if options["disk"]:
        lines.append("${color3}${voffset 3}${alignc}${font Dejavu Sans Mono:bold:size=10}Disks${font}")
        lines.append("${voffset -10}${color3}${goto 36}Used ${alignr 10}Free")
        lines.append("${color3}${goto 36}${fs_used /}${alignc -30}ROOT${alignr 15}${fs_free /}")
        lines.append("${alignc}${voffset 0}${color2}${fs_bar 10,220 /}${color}")
        lines.append("${color3}${goto 36}${fs_used /home/}${alignc -30}Home${alignr 15}${fs_free /home}")
        lines.append("${alignc}${voffset 0}${color2}${fs_bar 10,220 /home}${color}")
